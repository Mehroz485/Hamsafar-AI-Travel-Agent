...
# FIX: the outer "client = MultiServerMCPClient(" (and its closing ")" below)
# was nested around the inner call, which caused:
# TypeError: MultiServerMCPClient.__init__() got an unexpected keyword argument 'client'
...
client = MultiServerMCPClient(
    {
        "weather": {
            "transport": "stdio",
            # FIX: old hard-coded paths pointed to another machine/folder.
            # Now uses the currently running Python and the local custom_mcp.py.
            # "command": r"C:\Users\MALIK TAHIR\Desktop\AGENT\.venv\Scripts\python.exe",
            "command": sys.executable,
            "args": [
                # r"C:\Users\MALIK TAHIR\Desktop\AGENT\custom_mcp.py"
                str(WEATHER_SERVER_PATH)
            ],
            # FIX: pass the full environment so the subprocess works on Windows.
            # "env": {
